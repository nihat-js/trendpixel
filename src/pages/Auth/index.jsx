...
import {saveCredentials} from "../../utils/saveCredentials.js";
...
export function AuthPage() {
  ...
  function handleSubmit(e) {
    ...
    if (formData.password.length < 6) {
      toast.error('Password should be at least 6 charcters')
      return false
    }
    if (activeForm === 'LOGIN') {
      client.post('auth/login', {
        ...
        if (res?.data.status === 'success') {
          ...
          saveCredentials({username: formData.username, token: res.data.token}, formData.keep)
          navigate('/')
        }
      })
          ...
          <form action="">
            ...
            <div className="extra flex justify-between mt-7">
              <div className="keep-me flex gap-2">
                <input type="checkbox" id="" name="keep"  checked={formData.keep} onChange={() => setFormData({ ...formData , keep : !formData.keep }) }/>
